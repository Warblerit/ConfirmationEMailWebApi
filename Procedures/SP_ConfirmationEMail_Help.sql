...
-- ModifiedBy - ModifiedDate - Changes      
-- Anbu       - 18/02/2020   - Tac amount deducted from AgreedTariif for BTC Booking's
-- Prabhu     - 22/02/2020   - GST details added
--=================================================================================    
...
FROM WRBHBBookingProperty WHERE Id IN (SELECT TOP 1 BookingPropertyTableId FROM WRBHBBookingPropertyAssingedGuest WHERE BookingId = @Id);                                
  ...
  /* -- Get CPP & Property Mail */                      
	...
	CREATE TABLE #GST(MobileNo NVARCHAR(MAX));                                
	...
	DECLARE @TACFlag BIT = 0,@TACException BIT,@TACTaxInclu BIT;                                
                                
	/* -- NEW */                      
	SELECT TOP 1 @SingleTariff = P.AgreedSingle,@DoubleTariff = P.AgreedDouble,@TripleTariff = P.AgreedTriple,                                
	@TACFlag = ISNULL(TAC,0),@TACException=ISNULL(P.DiscountModeRS,0),@TACTaxInclu=ISNULL(P.TACTaxInclusive,0) ,                                
	...
	/* --CHANGES - MARKUP PERCENTAGE ISSUE PROCESS END */                      
      ...
      /* -- CHANGES Markup Amount not shown in BTC END (29 SEP 2015) */                      
		...
		END                                
		END            
			SET @AgreedTariff = @AgreedTariff + '<br> TAC : ' + CAST(@TACPer AS VARCHAR)+CASE WHEN @TACTaxInclu=0 THEN 
			' % + Taxes as applicable.'	ELSE '%  Nett' END;  
		END
		ELSE
		BEGIN
			IF @SingleCnt != 0                                 
			BEGIN                                
				SET @AgreedTariff = 'Single - Tariff : ' + CAST(CAST(@SingleTariff- (@SingleTariff * @TACPer)/100 AS DECIMAL(27,2)) AS VARCHAR)+', '                              
			END                                
			IF @DoubleCnt != 0                                
			BEGIN 
			IF @AgreedTariff != ''                                
			BEGIN 
				SET @AgreedTariff =@AgreedTariff + 'Double - Tariff : ' + CAST(CAST(@DoubleTariff- (@DoubleTariff * @TACPer)/100 AS DECIMAL(27,2)) AS VARCHAR)+ ', ';     
			END                                
			ELSE                                
			BEGIN                 
				SET @AgreedTariff = 'Double - Tariff : ' + CAST(CAST(@DoubleTariff- (@DoubleTariff * @TACPer)/100 AS DECIMAL(27,2)) AS VARCHAR)                               
			END
			END                                
			IF @TripleCnt != 0                                
		BEGIN
		...
		ELSE                                
		BEGIN                 
			SET @AgreedTariff = 'Triple - Tariff : ' + CAST(CAST(@TripleTariff- (@TripleTariff * @TACPer)/100 AS DECIMAL(27,2)) AS VARCHAR)
		END
		END  
		END
		END        
		...
		BEGIN
			IF @SingleCnt != 0                                 
			BEGIN                                
				SET @AgreedTariff = 'TAC Amount : ' + CAST(CAST((@SingleTariff * @TACPer)/100 AS DECIMAL(27,2)) AS VARCHAR)+', '                              
			END                                
			IF @DoubleCnt != 0                                
			BEGIN
			IF @AgreedTariff != ''                                
			BEGIN   
				SET @AgreedTariff =@AgreedTariff + 'TAC Amount : ' + CAST(CAST((@DoubleTariff * @TACPer)/100 AS DECIMAL(27,2)) AS VARCHAR)+ ', ';           
				 
			END 
			ELSE                                
			BEGIN  
				SET @AgreedTariff = 'TAC Amount : ' + CAST(CAST((@DoubleTariff * @TACPer)/100 AS DECIMAL(27,2)) AS VARCHAR);
			END                             
			END                                
			IF @TripleCnt != 0                                
			BEGIN 
			IF @AgreedTariff != ''                                
			BEGIN   
				SET @AgreedTariff =@AgreedTariff + 'TAC Amount : ' + CAST(CAST((@TripleTariff * @TACPer)/100 AS DECIMAL(27,2)) AS VARCHAR)+ ', ';           
				 
			END 
			ELSE                                
			BEGIN  
				SET @AgreedTariff = 'TAC Amount : ' + CAST(CAST((@TripleTariff * @TACPer)/100 AS DECIMAL(27,2)) AS VARCHAR);
			END                                  
	...
	BP.PropertyType IN ('ExP'))                                
	BEGIN                                
	DECLARE @Single DECIMAL(27,2),@Double DECIMAL(27,2),@Triple DECIMAL(27,2),@TaxIncluv BIT;                                
	
	SELECT @Single=CASE WHEN ISNULL(BP.DiscountModeRS,0)=0 THEN AgreedSingle - (AgreedSingle * TACPer/100) ELSE AgreedSingle END
	,@Double=CASE WHEN ISNULL(BP.DiscountModeRS,0)=0 THEN AgreedDouble - (AgreedDouble * TACPer/100) ELSE AgreedDouble END,
	@Triple=CASE WHEN ISNULL(BP.DiscountModeRS,0)=0 THEN AgreedTriple - (AgreedTriple * TACPer/100) ELSE AgreedTriple END,
	@TaxIncluv=TaxInclusive                              
	FROM WRBHBBookingProperty BP                                  
	LEFT OUTER JOIN WRBHBBookingPropertyAssingedGuest BG WITH(NOLOCK)ON                                
	BP.BookingId=BG.BookingId AND BP.Id=BG.BookingPropertyTableId AND                                
	BP.PropertyId=BG.BookingPropertyId                                
	...
	WHEN Occupancy = 'Double' THEN @Double                                
	WHEN Occupancy = 'Triple' THEN @Triple                                
	ELSE Tariff END AS VARCHAR)+'/-' + CASE WHEN @TaxIncluv=1 THEN '<br>  Nett'ELSE '<br>  + Taxes' END ELSE  
	CAST(CASE WHEN Occupancy = 'Single' THEN @Single                                
	WHEN Occupancy = 'Double' THEN @Double                                
	WHEN Occupancy = 'Triple' THEN @Triple                                
	...
	/* -- Dataset Table 12 bEGIN */ 
	IF EXISTS(SELECT NULL FROM WRBHBClientManagementGSTStatewise WHERE ClientId = @GSTClientId )            
	 BEGIN   
	  SELECT ISNULL(S.ClientName,'') AS ClientName,ISNULL(C.GSTNumber, '') AS GSTNumber,ISNULL(C.Address,'') AS Address         
	  FROM WRBHBClientManagementGSTStatewise C        
	  JOIN WRBHBClientManagement S ON C.ClientId=S.Id        
	  WHERE ClientId =@GSTClientId AND C.IsActive=1 AND C.IsDeleted=0 AND C.StateId=@GSTStateId    
     END        
	ELSE        
	 BEGIN
	  SELECT ISNULL(ClientName,'') AS ClientName,ISNULL('', '') AS GSTNumber,ISNULL('','') AS Address        
	  FROM WRBHBClientManagement WHERE Id=@GSTClientId        
	 END 
	 /* -- Dataset Table 12 eND */ 
...
IF @Action = 'BedBookingConfirmed'                                
 ...
 ISNULL(C.SupportEmail,'stay@hummingbirdindia.com') AS SupportEmail,@BookingConfirmBy FROM WRBHBBooking B                                
  ...
  CREATE TABLE #GST1(MobileNo NVARCHAR(MAX));                                
